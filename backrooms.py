...
class App(object):
    ...
    def demarc_nav_indicator(self):
        ...
        nf_min_x = self.config["window"]["size"][0] // 2 - \
            self.config["window"]["size"][0] * self.config["navigation"]["forward_region_width"] // 2
        nf_max_x = self.config["window"]["size"][0] // 2 + \
            self.config["window"]["size"][0] * self.config["navigation"]["forward_region_width"] // 2
        nf_min_y = self.config["window"]["size"][1] // 2 - \
            self.config["window"]["size"][1] * self.config["navigation"]["forward_region_width"] // 2
        nf_max_y = self.config["window"]["size"][1] // 2 + \
            self.config["window"]["size"][1] * self.config["navigation"]["forward_region_width"] // 2
        ...
        if x < ss_region_left and ss_min_y < y < ss_max_y and "left" in self.world.room.vars["exits"]:
            ...
            blit_loc = (self.config["window"]["size"][0] - self.images["chevron_right"].get_width() - pad,
                        self.config["window"]["size"][1] // 2 - self.images["chevron_right"].get_height() // 2)
            ...
            blit_loc = (self.config["window"]["size"][0] // 2 - self.images["chevron_down"].get_width() // 2,
                        self.config["window"]["size"][1] - self.images["chevron_down"].get_height() - pad)
            ...
            if "forward" in self.world.room.vars["exits"] and "backward" in self.world.room.vars["exits"]:
                blit_loc = (self.config["window"]["size"][0] // 2 - self.images["arrow_double"].get_width() // 2,
                            self.config["window"]["size"][1] // 2 - self.images["arrow_double"].get_height() // 2)
                self.screen.blit(self.images["arrow_double"], blit_loc)
                self.cursor.nav = "double"
            elif "forward" in self.world.room.vars["exits"]:
                blit_loc = (self.config["window"]["size"][0] // 2 - self.images["arrow_forward"].get_width() // 2,
                            self.config["window"]["size"][1] // 2 - self.images["arrow_forward"].get_height() // 2)
                self.screen.blit(self.images["arrow_forward"], blit_loc)
                self.cursor.nav = "forward"
            elif "backward" in self.world.room.vars["exits"]:
                blit_loc = (self.config["window"]["size"][0] // 2 - self.images["arrow_backward"].get_width() // 2,
                            self.config["window"]["size"][1] // 2 - self.images["arrow_backward"].get_height() // 2)
